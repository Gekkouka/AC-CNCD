...
#The return is a list of x and y, where the list contains eeg_data for each subject
...
# Load the eeg_data, return a list of samples and labels, including tensors of eeg_data for each subject
def load4train(samples_path_list, args):
    """
    load the SEED eeg_data set
    """
...
#The input is the full set of eeg_data from the first session.
def getDataLoaders(one_subject, args):
    ...
    try:
        target_path_list = [i for i in path_list if(i.startswith(config_path["file_path"] + str(int(one_subject))+"_"))]
