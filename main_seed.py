...
from train import trainMMDAAE
from utils import setup_seed


def main(log_file, data_loader_dict, optimizer_config_model, optimizer_config_adv, device, args, one_subject):
    if args.dataset_name == 'seed3':
        args.iteration = 12
    elif args.dataset_name == 'seed4':
        args.iteration = 3
    acc = trainMMDAAE(log_file, data_loader_dict, optimizer_config_model, optimizer_config_adv, device, args, one_subject)
    return acc

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='MMDAAE')

    parser.add_argument('--way', type=str, default='seed', help="experiment message for save model")
    ...
    parser.add_argument("--w_ae", type=float, default=0.40, help="weight for autoencoder loss")  # 0.5
    parser.add_argument("--w_cls", type=float, default=1, help="weight for classification loss")  # 0.15
    ...
    parser.add_argument("--output_dim", type=int, default=310, help="output dim is the same with sample's input dim for autoencoder")
    ...
    for one_subject in range(1, 16):
        ...
        # 2. main
        acc = main(log_file, data_loader_dict, optimizer_config_model, optimizer_config_adv, device, args, one_subject)
        acc_list.append(acc)
